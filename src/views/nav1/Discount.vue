<template>
	<section>
        ...
        <el-form-item>
				...
				<el-form-item>
					<el-button type="primary" v-on:click="getDiscounts">查询</el-button>
				</el-form-item>
        -->
				<el-form-item>
		...
		<!--列表-->
		<el-table :data="discounts" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			<el-table-column type="selection" style="width: 10%;">
			</el-table-column>
			<el-table-column type="index" width="60">
			</el-table-column>
			<el-table-column prop="info" label="优惠信息" sortable>
			</el-table-column>
			<el-table-column prop="store" label="商家名称" width="120" :formatter="formatStore" sortable>
			</el-table-column>
			<el-table-column prop="expire" label="截止日期" width="120" :formatter="formatDate" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="截止日期" prop="expire">
					<el-date-picker type="date" placeholder="选择日期" v-model="editForm.expire"></el-date-picker>
				</el-form-item>
				<el-form-item label="优惠信息" prop="info">
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				...
				-->
				<el-form-item label="商家" prop="store">
					<el-select v-model="addForm.store" placeholder="请选择商家">
				...
				<el-form-item label="截止日期" prop="expire">
					<el-date-picker type="date" placeholder="选择日期" v-model="addForm.expire"></el-date-picker>
				</el-form-item>
				<el-form-item label="优惠信息" prop="info">
...
<script>
	...
	import { getDiscountListPage, removeDiscount, batchRemoveDiscount, editDiscount, addDiscount, getStores } from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getDiscounts() {
				...
				//NProgress.start();
				return getDiscountListPage(para).then((res) => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						//this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							if(!(para.store && para.expire && (para.state === 0 || para.state === 1) && para.info)){
							...
							editDiscount(para).then((res) => {
								this.editLoading = false;
								//NProgress.done();
								if(res.data.error){
									this.$message({
										message: '修改失败，可能已添加过此商家信息',
										type: 'warning'
									});
									return;
								}
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						//this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							//NProgress.start();
							para.expire = (!para.expire || para.expire == '') ? '' : util.formatDate.format(new Date(para.expire), 'yyyy-MM-dd');
							addDiscount(para).then((res) => {
								this.addLoading = false;
								//NProgress.done();
								if(res.data.error){
									this.$message({
										message: '添加失败，可能已添加过此商家',
										type: 'warning'
									});
									return;
								}
			...
			batchRemove: function () {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
					...
					//NProgress.start();
					let para = { ids: ids };
					batchRemoveDiscount(para).then((res) => {
						this.listLoading = false;
						//NProgress.done();
						this.$message({
							message: '删除成功',
		...
		mounted() {
			this.getStores().then(() => this.getDiscounts());
		}
	}
